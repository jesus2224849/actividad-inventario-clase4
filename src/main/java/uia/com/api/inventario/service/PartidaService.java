...
public class PartidaService {
            ...
            for (int i = 0; i < partidasDTO.getPartidas().size(); i++)
            {
                ...
                else
                {
                    Optional<Partida> entityPartida =  this.repositoryPartidas.findById(id);
                        ...
                        if (!this.repositorySubpartidas.existsById(partidasDTO.getPartidas().get(i).getNameSubpartida()))
                        {
                            agregaSubpartida(entityPartida.get(), partidasDTO.getPartidas().get(i));
                        }
    ...
    private void agregaSubpartida(Partida entityPartida, PartidaDTO entityPartidaDTO)
    {
        String id = entityPartida.getNameSubpartida();
        String idDTO = entityPartidaDTO.getNameSubpartida();

        if (!this.repositorySubpartidas.existsById(idDTO))
        {
            Subpartida subpartida = new Subpartida(idDTO, idDTO, entityPartidaDTO.getIdSubpartida(), entityPartidaDTO.getEstatus(),"Subpartidas", "02-02-2023", "No definido");
            Set<ConstraintViolation<Subpartida>> violations = validator.validate(subpartida);
            if (!violations.isEmpty()) {
                throw new ConstraintViolationException(violations);
            }
